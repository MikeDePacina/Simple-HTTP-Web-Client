...
void parse_url(char *url, char **hostname, char **port, char **path){
	//look for index of ://, protocol is substring before that
	//if not http throw error
	
	//pointer to char used to iterate through chars of URL
	char *p;
	p = strstr(url, "://");
	...
	//now p should be pointing to hostname, hostname is until first colon, slash or hash
	
	*hostname = p;
	while(*p && *p != ':' && *p != '/' && *p != '#') p++;
	
	//now check for port number should be 80 cos its HTTP
